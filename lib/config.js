...
function getConfig() {
  const config = {
    ...
    chainId: Number(process.env.CHAIN_ID || '2522'),
    apiKey: process.env.API_KEY || 'secretToken',
    graphInstanceType: process.env.GRAPH_INSTANCE_TYPE || 't2.medium',
...
if (!ENV) {
  throw new Error(`Please set the environment variable ENV ${ENV}`)
}
