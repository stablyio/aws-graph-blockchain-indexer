...
class GraphNodeCluster extends Construct {
  ...
  constructor(scope, id, props) {
    ...
    // VPC Endpoint to SSM
    // TODO: We had to manually comment this line because the VPC for secret manager already exists.
   /* vpc.addInterfaceEndpoint(`secretsmanagerVPCE${ENV}`, {
    ...
    //   securityGroupName: `graph-node-alg-sg-${ENV}`,
    //   vpc: vpc,
    ...
    const cluster = new Cluster(this, `cluster-${ENV}`, {
      clusterName: `graph-node-${ENV}`,
      vpc: vpc,
      containerInsights: true,
      // enableFargateCapacityProviders: true
    })
    // cluster.node.addDependency(autoScalingGroup)

    // Persistent volume: EFS
    const fileSystem = new FileSystem(this, `efs-${ENV}`, {
      fileSystemName: `graph-node-efs-${ENV}`,
      vpc,
      removalPolicy: RemovalPolicy.DESTROY,
      encrypted: true,
      vpcSubnets: {
    ...
    const nodeClientRole = new Role(this, graphNodeClientRoleName, {
      roleName: `graph-node-client-role-${ENV}`,
      assumedBy: new ServicePrincipal('ec2.amazonaws.com'),
      managedPolicies: [
        // ECS must use this policy to register the EC2 instance with the ECS cluster
    ...
    const nodeClientLaunchTemplate = new LaunchTemplate(
      ...
      {
        launchTemplateName: `graph-node-client-launch-template-${ENV}`,
        ...
        // ECS only can connect to EC2 if the EC2 has a public IP
    ...
    const capacityProvider = new AsgCapacityProvider(this, `capacity-provider-${ENV}`, {
      capacityProviderName: `graph-node-asg-capacity-provider-${ENV}`,
    ...
    const graphnodeContainer = taskDefinition.addContainer(`graph-node-${ENV}`, {
      ...
      secrets: {
        ...
        postgres_db: Secret.fromSecretsManager(dbSecret, 'dbname'),
    ...
    const accessLogsBucket = new Bucket(this, `access-logs-${ENV}`, {
      bucketName: `graph-node-access-logs-${ENV}`,
    ...
    const service = new Ec2Service(this, `ec2-service-${ENV}`, {
      serviceName: `graph-node-service-${ENV}`,
      cluster: cluster,
    ...
    //   loadBalancerName: `graph-node-alb-80-${ENV}`,
    //   vpc: vpc,
    //   internetFacing: true,
    //   vpcSubnets: {
    //     subnetType: SubnetType.PUBLIC,
    ...
    //   targetGroupName: `graph-node-ecs-80-${ENV}`,
    //   port: 80,
    //   protocol: ApplicationProtocol.HTTP,
    ...
    //   loadBalancerName: `graph-node-alb-8030-${ENV}`,
    //   vpc: vpc,
    //   internetFacing: true,
    //   vpcSubnets: {
    //     subnetType: SubnetType.PUBLIC,
    ...
    //   targetGroupName: `graph-node-ecs-8030-${ENV}`,
    //   port: 8030,
    //   protocol: ApplicationProtocol.HTTP,
    //   targets: [autoScalingGroup],
    // })
    ...
    //   {
    //     id: 'AwsSolutions-EC23',
    //     reason: 'Security group allows traffic from the internet',
    //   },
    // ])
