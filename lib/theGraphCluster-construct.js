...
if (!ENV) {
  throw new Error('Please set the environment variable DEV')
}
...
class GraphNodeCluster extends Construct {
  ...
  constructor(scope, id, props) {
    ...
    const albSg = new SecurityGroup(this, `graph-node-alg-sg-${ENV}`, {
      vpc: vpc,
      description: `ALB SG ${ENV}`,
    })
    Tags.of(albSg).add('Name', `alg-sg-${ENV}`)

    // The Graph SG for ECS EC2 open for IPFS p2p communication and ALB SG
    const graphServiceSg = new SecurityGroup(this, `graph-node-ec2-sg-${ENV}`, {
      vpc: vpc,
      description: `graph-node-sg-${ENV}`,
    })
    Tags.of(graphServiceSg).add('Name', `graph-node-sg-${ENV}`)

    const dbName = `graph-node-db-${ENV}`
    ...
    const dbSecret = new secretsmanager.Secret(this, `graph-node-db-secret-${ENV}`, {
      secretObjectValue: {
        host: '',
        port: '5432',
        dbname: dbName,
    ...
    const createDBLambda = new NodejsFunction(this, `create-graph-node-db-lambda-${ENV}`, {
    ...
    const createDBCustomResourceProvider = new Provider(
      this,
      `createGraphNodeDBCustomResourceProvider${ENV}`,
      {
    ...
    // ECS Cluster
    const cluster = new Cluster(this, `GraphNodeEc2Cluster${ENV}`, {
      vpc: vpc,
      containerInsights: true,
    })

    // Persistent volume: EFS
    const fileSystem = new FileSystem(this, `GraphNodeEfsFileSystem${ENV}`, {
      vpc,
      removalPolicy: RemovalPolicy.DESTROY,
      encrypted: true,
      vpcSubnets: {
    ...
    const nodeClientRole = new Role(this, `graphNodeClientRole${ENV}`, {
    ...
    const nodeClientLaunchTemplate = new LaunchTemplate(
      this,
      `graphNodeClientLaunchTemplate${ENV}`,
      {
    ...
    const taskDefinition = new Ec2TaskDefinition(this, `GraphNodeTaskDef${ENV}`, {
      volumes: [
        {
          name: efsVolumeName,
    ...
    const ipfsContainer = taskDefinition.addContainer(`ipfs${ENV}`, {
    ...
    // access log bucket
    const accessLogsBucket = new Bucket(this, `graph-node-access-logs-${ENV}`, {
    ...
    const service = new Ec2Service(this, `graph-node-ec2-Service-${ENV}`, {
      cluster: cluster,
    ...
    const alb_port80 = new ApplicationLoadBalancer(this, `graph-node-alb-port80-${ENV}`, {
      vpc: vpc,
    ...
    const tg_p80 = listener_port80.addTargets(`graph-node-ecs-${ENV}`, {
      port: 80,
      protocol: ApplicationProtocol.HTTP,
    ...
    const alb_port8030 = new ApplicationLoadBalancer(this, `graph-node-alb-port8030-${ENV}`, {
      vpc: vpc,
      internetFacing: false,
      vpcSubnets: {
        subnetType: SubnetType.PUBLIC,
    ...
    const tg_p8030 = listener_port8030.addTargets(`graph-node-ecs-${ENV}`, {
      port: 8030,
      protocol: ApplicationProtocol.HTTP,
      targets: [autoScalingGroup],
    })
    ...
    NagSuppressions.addResourceSuppressionsByPath(
      this,
      'GraphNodeServiceStack/GraphCluster/DbCluster/Secret/Resource',
      [
        {
    ...
    NagSuppressions.addResourceSuppressionsByPath(
      this,
      'GraphNodeServiceStack/GraphCluster/DbCluster/Resource',
      [
        {
    ...
    NagSuppressions.addResourceSuppressionsByPath(
      this,
      [
        'GraphNodeServiceStack/GraphCluster/NodeClientRole/DefaultPolicy/Resource',
        '/GraphNodeServiceStack/GraphCluster/createDBCustomResourceProvider/framework-onEvent/ServiceRole/DefaultPolicy/Resource',
        '/GraphNodeServiceStack/GraphCluster/NodeClientRole/DefaultPolicy/Resource',
        '/GraphNodeServiceStack/GraphCluster/ASG/DrainECSHook/Function/ServiceRole/DefaultPolicy/Resource',
        '/GraphNodeServiceStack/GraphCluster/ASG/DrainECSHook/Function/ServiceRole/DefaultPolicy/Resource',
      ],
      [
        {
    ...
    NagSuppressions.addResourceSuppressionsByPath(
      this,
      'GraphNodeServiceStack/GraphCluster/ASG/ASG',
      [
        {
          id: 'AwsSolutions-AS3',
          reason:
    ...
    NagSuppressions.addResourceSuppressionsByPath(
      this,
      [
        '/GraphNodeServiceStack/GraphCluster/createDBCustomResourceProvider/framework-onEvent/ServiceRole/Resource',
        '/GraphNodeServiceStack/GraphCluster/ASG/DrainECSHook/Function/ServiceRole/Resource',
        '/GraphNodeServiceStack/GraphCluster/NodeClientRole/Resource',
        '/GraphNodeServiceStack/GraphCluster/createDBLambda/ServiceRole/Resource',
      ],
      [
        {
    ...
    NagSuppressions.addResourceSuppressionsByPath(
      this,
      [
        'GraphNodeServiceStack/GraphCluster/ASG/DrainECSHook/Function/Resource',
        'GraphNodeServiceStack/GraphCluster/createDBCustomResourceProvider/framework-onEvent/Resource',
      ],
      [
        {
          id: 'AwsSolutions-L1',
    ...
    NagSuppressions.addResourceSuppressionsByPath(
      this,
      'GraphNodeServiceStack/GraphCluster/ASG/LifecycleHookDrainHook/Topic/Resource',
      [
        {
    ...
    NagSuppressions.addResourceSuppressionsByPath(
      this,
      'GraphNodeServiceStack/GraphCluster/GraphNodeTaskDef/Resource',
      [
        {
